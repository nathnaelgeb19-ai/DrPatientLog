...
def _validate_csrf():
    if request.method == "POST" and request.endpoint not in {"login", "setup", "forgot_password", "reset_password"}:
        ...
        if not token or not expected or not secrets.compare_digest(token, expected):
            # Prefer a recoverable UX over a bare 400 page (common after
            # session expiry or when JS CSRF injection did not run).
            if request.accept_mimetypes.accept_html:
                flash("Session expired or security token missing. Please try again.", "error")
...
def monthly_send():
    ...
    if month and year_raw:
        ...
        try:
            ...
            flash("Invalid Ethiopian year.", "error")
            return redirect(url_for("monthly"))
        # Day is unused by the filter; 1 is a stable placeholder.
        eth_date_str = f"{month} 1 {year}"
    text, label = build_monthly_report(doctor_id, eth_date_str)
    if not text:
...
def monthly_report_html():
    ...
    if month_q and year_q and month_q in ETH_MONTHS:
        try:
            m, y = month_q, int(year_q)
        except ValueError:
    ...
    if m == ETH_MONTHS[0]:
        pagume_carry = sum(
            float(r["my_earning"] or 0)
            for r in rows
            if r["eth_date"] and ETH_MONTHS[-1] in r["eth_date"] and str(y - 1) in r["eth_date"]
        )
    payable = cut + pagume_carry
    pct = (cut / income * 100.0) if income else 0.0
    rows_html = "".join(
        f"<tr><td>{r['eth_date']}</td><td>{r['patient_name']}</td><td>{r['card_number'] or ''}</td>"
